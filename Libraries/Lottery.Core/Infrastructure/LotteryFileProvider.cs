...
{
    public class LotteryFileProvider : PhysicalFileProvider, ILotteryFileProvider
    {
        public LotteryFileProvider(ILotteryEnvironment environment)
            : base(
                  File.Exists(environment.RootPath) ? 
                  Path.GetDirectoryName(environment.RootPath) 
                : environment.RootPath)
        {
            WebRootPath = File.Exists(environment.RootPath)
                ? Path.GetDirectoryName(environment.RootPath)
                : environment.RootPath;
        }
